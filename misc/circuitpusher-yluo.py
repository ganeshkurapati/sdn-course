...
NOTE by yluo 02/12/17:
1. Had to add ['device'] to json parsing due to the json format change in FloodLight
...
if args.action=='add':
    ...
    pprint (parsedResult)
    try:
        sourceSwitch = parsedResult['devices'][0]['attachmentPoint'][0]['switch']
    except IndexError:
    ...
    sourcePort = parsedResult['devices'][0]['attachmentPoint'][0]['port']
    ...
    try:
        destSwitch = parsedResult['devices'][0]['attachmentPoint'][0]['switch']
    except IndexError:
    ...
    destPort = parsedResult['devices'][0]['attachmentPoint'][0]['port']
    ...
    for i in range(len(parsedResult)):
        if i % 2 == 0:
            ap1Dpid = parsedResult['devices'][i]['switch']
